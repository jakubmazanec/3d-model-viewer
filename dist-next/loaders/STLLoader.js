...
function STLLoader(manager) {
	this.manager = manager === undefined ? _three2.default.DefaultLoadingManager : manager;
}

STLLoader.prototype = {

	constructor: _three2.default.STLLoader,

	load(url, onLoad, onProgress, onError) {
		let scope = this;
		let loader = new _three2.default.XHRLoader(scope.manager);
	...
	parse(data) {
		let isBinary = function isBinary() {
	...
	parseBinary(data) {
		...
		let offset = 0;
		let geometry = new _three2.default.BufferGeometry();
	...
	parseASCII(data) {
		...
		while ((result = patternFace.exec(data)) !== null) {
			...
			while ((result = patternNormal.exec(text)) !== null) {
				normal = new _three2.default.Vector3(parseFloat(result[1]), parseFloat(result[3]), parseFloat(result[5]));
			}

			patternVertex = /vertex[\s]+([\-+]?[0-9]+\.?[0-9]*([eE][\-+]?[0-9]+)?)+[\s]+([\-+]?[0-9]*\.?[0-9]+([eE][\-+]?[0-9]+)?)+[\s]+([\-+]?[0-9]*\.?[0-9]+([eE][\-+]?[0-9]+)?)+/g;

			while ((result = patternVertex.exec(text)) !== null) {
				geometry.vertices.push(new _three2.default.Vector3(parseFloat(result[1]), parseFloat(result[3]), parseFloat(result[5])));
			}

			length = geometry.vertices.length;

			geometry.faces.push(new _three2.default.Face3(length - 3, length - 2, length - 1, normal));
		}
